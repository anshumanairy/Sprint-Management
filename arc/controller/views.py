...
def qaprg(request):
    ...
    return(render(request,'qaprg.html/',{'data':data,'list1':list1,'p':p,'a':a,'b':b,'c':c,'d':d,'e':e,'f':f,'d1':jd1,'d2':jd2,'d3':jd3,'z':z}))
...
def view_story(request):
    ...
    if request.method=='GET':
        ...
        if 'edit1' in request.GET:
            ...
            p.save()
    if request.method=='POST' :
        if 'story_button' in request.POST:
...
def tasks(request):
    ...
    id = request.session['id']
    data = story.objects.filter(sprint_id=id)
    return(render(request,'tasks.html/',{'data':data,'z':z}))
